...
import re
from .data_preprocessing_imdb import build_prompt, infer_prompt_format_from_model_id
...
def evaluate_imdb(
    ...
    **kwargs,
    ...
    """
    Evaluate model on IMDB sentiment analysis.
    Returns:
    ...
    # Deterministic stratified sub-sampling from test for speed and balance
    ...
    cfg_id = getattr(cfg, "_name_or_path", "") if cfg is not None else ""
    model_id = (prompt_model_id or cfg_id or "").strip()
    prompt_format = infer_prompt_format_from_model_id(model_id)
    
    iterator = tqdm(dataset, desc="Evaluating IMDB") if progress_bar else dataset
