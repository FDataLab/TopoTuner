#!/usr/bin/env python3
"""
3 models × 3 datasets (SST-2, IMDB, MMLU) — Spectrum **top-N%** finetuning (default N=5), then post-train eval.

**Training** defaults match ``numpy_weights/exploration-finetuning/scripts/run_sst2_imdb_mmlu_finetune_run1.sh``:
SST2/MMLU train subset 20k, IMDB 25k; per-step batch SST2=64, MMLU=32, IMDB=``--batch-imdb`` (32);
LRs qwen 1e-5, llama 2e-5, mistral 5e-6 (same as that script’s *_LR_FULL).

**Eval (default ``--eval-backend cf3``)** runs ``codes/gsm8k/eval_catastrophic_forgetting3.py`` on the **task’s**
benchmark only (``sst2`` / ``imdb`` / ``mmlu``), with **K disjoint random eval folds** (default ``--eval-num-folds 3``)
and cached indices under ``<root>/eval_split_indices/`` — mean accuracy ± std across folds.

Optional ``--eval-backend run1`` matches ``run_sst2_imdb_mmlu_finetune_run1.sh`` ``run_final_eval`` (single full
greedy pass via ``codes.sst2|imdb|mmlu.eval_*``; no fold std).  CF3 uses GLUE SST-2, ``imdb`` test, and MMLU
**test** with its own prompts — use ``run1`` only when you need strict parity with that shell eval.

Spectrum YAMLs
  Written under ``competitor/spectrum-repo/`` as
  ``snr_results_<model-slug>_unfrozenparameters_<N>percent.yaml`` using the same per-type
  top-percent rule as ``spectrum-repo/spectrum.py`` (requires precomputed SNR JSON under
  ``spectrum-repo/model_snr_results/``).

Default models (override with ``--models`` space-separated HF ids):
  meta-llama/Llama-3.1-8B  Qwen/Qwen3-8B-Base  mistralai/Mistral-7B-v0.3

Example
  python run_competitor_spectrum_tri_task_grid.py --phase train --top-percent 5 --seed 42
  python run_competitor_spectrum_tri_task_grid.py --phase eval --eval-batch-size 32

Paths assume this file lives in
``…/numpy_weights/exploration-finetuning/competitor/``.  Python imports resolve
``…/codes`` by walking parents for ``codes/gsm8k/finetune_gsm8k_competitor.py``, then
``$HOME/topo/codes``, then ``$CODES_ROOT`` / ``$TOPO_CODES`` if set (needed when
``numpy_weights`` is symlinked under an external path without a sibling ``codes/`` tree).
"""
...
SPECTRUM_REPO = COMPETITOR_ROOT / "spectrum-repo"
SNR_JSON_DIR = SPECTRUM_REPO / "model_snr_results"
...
# Import task + gsm8k helper modules from ``<TOPO>/codes`` (script dir is competitor/, not on PYTHONPATH).
_codes = str(CODES_ROOT)
if _codes not in sys.path:
    sys.path.insert(0, _codes)

from gsm8k.finetune_gsm8k_competitor import (  # noqa: E402
    apply_spectrum_freezing,
    count_params,
    get_gpu_report,
    load_spectrum_unfrozen_parameters,
...
def snr_json_path(model_id: str) -> Path:
    return SNR_JSON_DIR / f"snr_results_{model_slug(model_id)}.json"


def spectrum_yaml_path(model_id: str, top_percent: int) -> Path:
    base = f"snr_results_{model_slug(model_id)}_unfrozenparameters_{top_percent}percent.yaml"
    return SPECTRUM_REPO / base


def write_spectrum_unfrozen_yaml_from_snr_json(
    snr_json: Path,
    top_percent: int,
    out_yaml: Path,
) -> None:
    """Mirror ``spectrum-repo/spectrum.py::generate_unfrozen_params_yaml``."""
    with open(snr_json, encoding="utf-8") as f:
        snr_data: Dict[str, Any] = json.load(f)

    unfrozen_by_type: Dict[str, List[Tuple[str, float]]] = {}
    for layer_name, info in snr_data.items():
        ...
        if raw is None or (isinstance(raw, float) and math.isnan(raw)):
            ...
            try:
                ...
                snr_f = float("-inf")
        unfrozen_by_type.setdefault(lt, []).append((layer_name, snr_f))

    top_layers_by_type: Dict[str, List[str]] = {}
    for layer_type, layers in unfrozen_by_type.items():
        layers_sorted = sorted(layers, key=lambda x: x[1], reverse=True)
        n = int(len(layers) * top_percent / 100)
        top_layers_by_type[layer_type] = [x[0] for x in layers_sorted[:n]]

    out_yaml.parent.mkdir(parents=True, exist_ok=True)
    with open(out_yaml, "w", encoding="utf-8") as file:
...
def ensure_spectrum_yaml(model_id: str, top_percent: int, *, force_regenerate: bool) -> Path:
    js = snr_json_path(model_id)
    if not js.is_file():
        raise FileNotFoundError(f"Missing SNR JSON (run spectrum scan first): {js}")
    yml = spectrum_yaml_path(model_id, top_percent)
    if yml.is_file() and not force_regenerate:
        return yml
    write_spectrum_unfrozen_yaml_from_snr_json(js, top_percent, yml)
    return yml
...
def model_short_for_tri_grid(model_id: str) -> str:
    """Labels used in ``run_sst2_imdb_mmlu_finetune_run1.sh`` (RESULTS_DIR / eval batch routing)."""
    low = model_id.lower()
    if "qwen" in low:
        return "qwen-base"
    if "mistral" in low:
...
def lr_for_run1_full_finetune(model_id: str) -> float:
    """Same table as ``run_sst2_imdb_mmlu_finetune_run1.sh`` (QWEN_LR_FULL / LLAMA_LR_FULL / MISTRAL_LR_FULL)."""
    ms = model_short_for_tri_grid(model_id)
    if ms == "qwen-base":
        return 1e-5
    if ms == "mistral-7b-v03":
        return 5e-6
    return 2e-5


def train_batch_for_task(task: str, batch_imdb: int) -> int:
    """Per-dataset train microbatch defaults from ``run_sst2_imdb_mmlu_finetune_run1.sh``."""
    if task == "sst2":
        return 64
    if task == "mmlu":
        return 32
...
def timing_extrapolation_note(training_min: float, subset_fraction: float) -> Dict[str, Any]:
    """Rough scaling for planning full-data reruns (same epochs, linear-rows heuristic)."""
    return {
...
def _collect_checkpoints(out_root: Path) -> List[Path]:
    """Match ``run_sst2_imdb_mmlu_finetune_run1.sh`` ``run_final_eval`` checkpoint pick (epoch_weights or checkpoint-*)."""
...
def run_final_eval_like_run1(
    *,
    ...
    """
    Post-train eval aligned with ``scripts/run_sst2_imdb_mmlu_finetune_run1.sh`` → ``run_final_eval``:
    tokenizer from ``base_model``, weights from last checkpoint under ``run_dir``,
    ``codes.sst2|imdb|mmlu.eval_*`` on the same splits (SST2 val / IMDB test / MMLU val).

    Writes ``<run_dir>/final_eval_<task>_run1_style.json``.

    Note: optional ``EVAL_NUM_FOLDS`` in the shell expects extra keys from ``evaluate_*``;
    those helpers currently do not implement k-fold; this path runs a **single full greedy pass**
    (same as unset ``EVAL_NUM_FOLDS`` in the shell). For k-fold + batched generate use ``--eval-backend cf3``.
    """
...
def train_one(
    *,
    ...
    lr_use = float(lr) if lr is not None else lr_for_run1_full_finetune(model_id)

    output_dir.mkdir(parents=True, exist_ok=True)
    report_path = output_dir / "training_report_spectrum.json"
    if report_path.is_file():
        print(f"[skip train] exists: {report_path}", flush=True)
        return report_path

    print(f"\n{'=' * 70}\n  TRAIN  task={task}  model={model_id}\n{'=' * 70}", flush=True)
    ...
    patterns = load_spectrum_unfrozen_parameters(str(spectrum_yaml))
    selected = apply_spectrum_freezing(model, patterns, target_modules=None)
    print(f"  Spectrum yaml: {spectrum_yaml}", flush=True)
    print(f"  Patterns: {len(patterns)}  |  matched trainable tensors: {len(selected)}", flush=True)
    ...
    report: Dict[str, Any] = {
        "experiment": f"{task.upper()} spectrum finetuning (tri-task grid)",
        "task": task,
        "model": model_id,
        "method": "spectrum",
        "top_percent": int(re.search(r"(\d+)percent", spectrum_yaml.name).group(1)) if re.search(r"(\d+)percent", spectrum_yaml.name) else None,
        ...
        "spectrum_config": {
            "unfrozen_file": str(spectrum_yaml),
            "num_selected_tensors": len(selected),
            "selected_tensors_preview": selected[:50],
        },
        "model_info": {
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Spectrum top-N%: 3×3 SST2/IMDB/MMLU train + eval (run1 or CF3).")
    ...
    ap.add_argument("--top-percent", type=int, default=5)
    ap.add_argument("--root-out-dir", type=str, default=None, help="Parent for all runs (default: competitor/spectrum_tri_task_top{N}/<stamp>/)")
    ap.add_argument("--stamp", type=str, default=None, help="Output folder stamp (default: UTC ymd_HMS).")
    ap.add_argument("--epochs", type=int, default=6)
    ap.add_argument("--grad-accum", type=int, default=1)
    ap.add_argument("--subset-size", type=int, default=20_000, help="SST2 + MMLU train rows (matches run1 SUBSET_SIZE).")
    ap.add_argument("--imdb-subset-size", type=int, default=25_000, help="IMDB train rows (matches run1 SUBSET_SIZE_IMDB).")
    ap.add_argument(
        "--subset-fraction",
        type=float,
        default=1.0,
        help="Multiply SST/MMLU and IMDB train subset caps by this factor (e.g. 0.25 for ¼ data). "
        "timing_extrapolation in each training_report estimates full-cap runtime.",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="If omitted, uses run1 full-finetune LR table (1e-5 qwen / 2e-5 llama / 5e-6 mistral).",
    )
    ap.add_argument("--max-length", type=int, default=None, help="Per-task default if omitted: sst2=256, imdb/mmlu=512.")
    ap.add_argument("--logging-steps", type=int, default=5)
    ap.add_argument("--regenerate-spectrum-yaml", action="store_true")
    ap.add_argument(
        "--eval-backend",
        choices=("run1", "cf3"),
        default="cf3",
        help="cf3 (default): eval_catastrophic_forgetting3.py, K-fold eval on the matching benchmark only. "
        "run1: eval_sst2/eval_imdb/eval_mmlu single-pass like run_sst2_imdb_mmlu_finetune_run1.sh.",
    )
    ap.add_argument("--eval-batch-llama-qwen", type=int, default=64, help="Post-train eval batch (run1 EVAL_BATCH_LLAMA_QWEN).")
    ap.add_argument("--eval-batch-mistral", type=int, default=128, help="Post-train eval batch (run1 EVAL_BATCH_MISTRAL).")
    ap.add_argument("--eval-batch-size", type=int, default=None, help="If set, overrides per-family eval batch for run1 backend.")
    ap.add_argument("--eval-max-new-tokens", type=int, default=8, help="Generation cap at eval (run1 EVAL_MAX_NEW_TOKENS default).")
    ...
    ap.add_argument("--eval-split-seed", type=int, default=42, help="CF3 only: --eval-split-seed.")
    ap.add_argument("--eval-num-folds", type=int, default=3, help="CF3 only: --eval-num-folds (default 3).")
    ap.add_argument(
        "--eval-split-indices-dir",
        type=str,
        default=None,
        help="CF3 only: shared dir for cached k-fold JSON (default: <root>/eval_split_indices).",
    )
    ap.add_argument("--eval-seed", type=int, default=None, help="CF3 only: --seed.")
    ap.add_argument("--max-samples", type=int, default=None, help="CF3 only: --max-samples per benchmark.")
    ...
    if args.root_out_dir:
        ...
        root = COMPETITOR_ROOT / f"spectrum_tri_task_top{args.top_percent}" / stamp
    ...
    for m in models:
        for task in args.tasks:
            slug = model_slug(m)
            run_dir = root / slug / task / f"{task}-spectrum-top{args.top_percent}-e{args.epochs}-{stamp}"
    ...
    yamls: Dict[str, Path] = {}
    for m in models:
        yp = spectrum_yaml_path(m, args.top_percent)
        yamls[m] = yp
        if args.dry_run:
            print(f"[dry-run] would ensure yaml: {yp}", flush=True)
            continue
        ensure_spectrum_yaml(m, args.top_percent, force_regenerate=args.regenerate_spectrum_yaml)
        print(f"Spectrum yaml OK: {yp}", flush=True)
