...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help='Top percentage of layers to select within each weight-type bucket (e.g. 50 = top 50%% SNR per type)',
    )
    parser.add_argument(
        '--batch-size',
        type=int,
        default=1,
        help='Layer batch size during SNR scan (only used when computing SNR, not for training)',
    )
    parser.add_argument(
        '--non-interactive',
        action='store_true',
        help='Skip TUI prompts; use --batch-size and scan all discovered weight types (for servers/nohup)',
    )
    ...
    if os.path.exists(snr_file_path):
        ...
        modifier = ModelModifier(model_name=None, top_percent=args.top_percent, batch_size=args.batch_size)
        ...
        if args.non_interactive:
            ...
            print(f"Non-interactive scan: batch_size={batch_size}", flush=True)
        else:
            batch_size_raw = input_dialog(title="Batch Size", text="Enter the batch size:").run()
            batch_size = int(batch_size_raw) if batch_size_raw else 1
        ...
        if args.non_interactive:
            ...
            print(f"Scanning {len(selected_weight_types)} weight types (all discovered).", flush=True)
        else:
