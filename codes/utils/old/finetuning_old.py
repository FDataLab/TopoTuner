...
def main():
    ...
    train_dataset = train_split["train"].select(range(len(train_split["train"]) // 4))
    val_dataset = train_split["test"].select(range(len(train_split["test"]) // 4))
    ...
    model_name = "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"
    model,tokenizer = load_model(model_name, device="cuda:0", use_lora=args.use_lora)
