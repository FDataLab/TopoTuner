...
def concise_lora_filename(param_name: str) -> str | None:
    """
    Match LoRA A/B on q/k/v projections and produce a short filename.
    Example: layers.7.self_attn.q_proj.lora_A.default.weight -> layer7_q_A.npy
    """
    m = re.search(r"layers\.(\d+)\.self_attn\.(q|k|v)_proj\.lora_(A|B)\.default\.weight", param_name)
    if m:
...
def concise_full_filename(param_name: str) -> str | None:
    ...
    """
    m = re.search(r"model\.layers\.(\d+)\.self_attn\.(q|k|v)_proj\.weight", param_name)
    if m:
        layer, proj = m.groups()
