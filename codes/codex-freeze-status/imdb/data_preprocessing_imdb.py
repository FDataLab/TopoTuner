...
# IMDB System prompt for sentiment analysis
IMDB_SYSTEM_PROMPT = """You are a helpful assistant that analyzes movie reviews and determines their sentiment. 
Given a movie review, classify it as either "positive" or "negative"."""
...
def build_prompt(tokenizer, text: str, prompt_format: str, use_instruction: bool = True) -> str:
    if prompt_format == "llama3":
        return create_prompt_llama3(tokenizer, text, use_instruction=use_instruction, prompt_template=IMDB_SYSTEM_PROMPT)
    elif prompt_format == "llama2":
        return create_prompt_llama2(text, use_instruction=use_instruction, prompt_template=IMDB_SYSTEM_PROMPT)
    elif prompt_format == "mistral":
        return create_prompt_mistral(text, use_instruction=use_instruction, prompt_template=IMDB_SYSTEM_PROMPT)
    elif prompt_format == "olmo":
        return create_prompt_olmo(text, use_instruction=use_instruction, prompt_template=IMDB_SYSTEM_PROMPT)
    else:  # "qwen"
        return create_prompt_qwen(tokenizer, text, use_instruction=use_instruction, prompt_template=IMDB_SYSTEM_PROMPT)
